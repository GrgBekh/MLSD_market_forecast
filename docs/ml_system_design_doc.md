...
### Связь метрик
MAPE, WAPE, bias - технические метрики точности прогноза

inaccuracy - что-то среднее между технической метрикой и бизнесовой

размен - бизнесовая метрика

Вопрос - как конкретно метрики прогноза влияют на бизнесовые метрики?

Введем понятие покрытия - доля товаров которые есть в кандидатах на предложение мерчам и при этом штук товара хватило на предложение
(например суммарное предложение мерчу по одному товару должно быть >100к рублей, иначе ему не выгодно это все нам везти)

Мы не можем пообещать мерчу что он продаст один столько же товаров, сколько мы предсказали будет продано за следующий месяц
но мы можем пообещать долю, скажем, 10%, эта доля должна быть >100к рублей

Очевидно у нас есть tradeoff между перепрогнозом предложения (можно же предлагать более 10% в нашей модели это настраиваемый параметр)
и покрытием предложения.
Чем точнее прогноз тем оптимальнее настраивается этот tradeoff.
...
Эксперименты по подбору гиперпараметров велись во внутреннем инструменте nirvana (для не посвященных это что то вроде airflow)
и выгдядит этот граф вот так

![Model fit DAG](./assets/Fit_graph.png)

Что происходит в графе:
- Собирается бинарь маппера. Mapper - специальная функция (а скорее класс) который принимает на вход произвольные данные и возвращает произвольные данные
маппер должен возвращать итерируемый объект - это нужно для mapreduce операций. Наш маппер считает статистические характеристики из рядов (перечисленны выше)
- Собирается лог заказов и подается в mapper
- Статистические фичи джойнятся с фичами воронки, dgmv_ratio, и native_pred
- Train test split на [begin_date, end_date - horizon), [end_date - horizon, end_date]
- Фичи приводятся в специальный формат для внутреннего катбуста - строка вида "<значение фичи>\t"
- Собирается таблица с описанием фичей в формате <тип фичи> <название фичи> <ее порядковый номер>
- Данные из двух предыдущих пунктов подаются в катбуст
- Собираются метрики обучения (показаны выше в [Валидации](###Валидация))
- Моделька за определенную логируется

Этот граф запускается раз в неделю

### Гиперпараметры
Пространство гиперпараметров - (learning_rate, n_iterations, l2_leaf_reg, depth)
Перебор ведется gridsearch, также пробовали optuna - долго
Результат перебора - (learning_rate: 0.121, n_iterations: 542, l2_leaf_reg: 0.1, depth: 6)
Отмечу что сильного влияния гиперпараметры на модель не оказывают, поэтому они зафиксированны для каждого планового переобучения модели

### Граф инференса

Граф инференса крутится в той же nirvana и выглядит так

![Model inference DAG](./assets/Inference_graph.png)

Сбор данных аналогичен графу для обучения, считаются лаговые метрики, о которых будет рассказано далее.
Этот граф запускается каждый день

### Мониторинги

Под отслеживание качества модели написаны мониторинги, они работают следующим образом:
Мы дали прогноз на 20000 товаров, из них фильтры прошли 1000, для этой 1000 товаров месяц назад
была прерасчитаны все метрики, мы не можем посчитать эти метрики сейчас но можем взять метрики за месяц назад
и посчитать их средние значения не на 20000 товаров, а только на 1000, так как остальные нам не интересны

![Monitorings](./assets/monitorings.png)

### Быстродействие

Очевидное требование - инференс должен длится меньше дня

Пожелание - к началу рабочего дня (а лучше и раньше) должен быть прогноз чтобы все потребители его подхватили

Благодаря mapreduce архитектуре решения граф инференса бежит 45 минут

![Monitorings](./assets/Inference_speed.png)

У нас подходящих товаров порядка 35000, а что если увеличить данные на входе в 20 раз?

Квота выдерживает сильно больше, скорость не упадет даже вдвое благодаря распределенным вычислениям,
применение катбуста к фичам делается также в маппере на разных машинах.
